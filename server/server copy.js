...
const sessionManager = require('./modules/session-utils');
const PPTConverter = require('./modules/PPTConverter');
const PDFImageConverter = require('./modules/PDFImageConverter');
const ContentExtractor = require('./modules/ContentExtractor');
const ContentAnalyzer = require('./modules/ContentAnalyzer');
const ScriptGenerator = require('./modules/ScriptGenerator');
const AudioGenerator = require('./modules/AudioGenerator');
const VideoGenerator = require('./modules/VideoGenerator');
const SubtitleGenerator = require('./modules/SubtitleGenerator');
...
class Server {
    ...
    async initializeMiddlewares() {
        ...
        this.app.use('/sessions', express.static('sessions'));
    ...
    setupRoutes() {
        this.app.post('/api/analyze-ppt', this.upload.single('file'), this.handlePPTAnalysis.bind(this));
        this.app.use(express.json()); // JSON 파싱을 위한 미들웨어 추가
        this.app.post('/api/update-script', this.handleScriptUpdate.bind(this));
        this.app.get('/api/get-script', this.handleGetScript.bind(this));
        this.app.post('/api/generate-slide-audio/:slideNumber', this.handleSlideAudioGeneration.bind(this));
        this.app.post('/api/merge-audio', this.handleAudioMerge.bind(this));
        this.app.get('/api/process-status', this.handleProcessStatus.bind(this));
        this.app.get('/api/session-info/:sessionId', this.handleSessionInfo.bind(this));

        // 오디오 파일 확인 API 추가
        this.app.get('/api/check-audio-files', this.handleCheckAudioFiles.bind(this));

        // 오디오 ZIP 다운로드 엔드포인트 추가
        this.app.get('/api/download-audio-zip', this.handleAudioZipDownload.bind(this));

        // 비디오 관련 API 엔드포인트 추가
        this.app.post('/api/generate-slide-video/:slideNumber', this.handleSlideVideoGeneration.bind(this));
        this.app.post('/api/generate-full-video', this.handleFullVideoGeneration.bind(this));
        this.app.get('/api/check-video-files', this.handleCheckVideoFiles.bind(this));
        this.app.get('/api/download-video-zip', this.handleVideoZipDownload.bind(this));

        // 자막 관련 API 엔드포인트 추가
        this.app.post('/api/generate-slide-subtitle/:slideNumber', this.handleSlideSubtitleGeneration.bind(this));
        this.app.post('/api/merge-subtitles', this.handleSubtitleMerge.bind(this));
        this.app.get('/api/check-subtitle-files', this.handleCheckSubtitleFiles.bind(this));
        this.app.get('/api/download-subtitle-zip', this.handleSubtitleZipDownload.bind(this));

        // 세션별 페이지 접근을 위한 라우트 추가
        this.app.get('/:sessionId', this.handleSessionPage.bind(this));

        this.app.post('/api/regenerate-slide', this.handleSlideRegeneration.bind(this));

        // 라우트 추가
        this.app.get('/api/download-all-content-zip', this.handleAllContentZipDownload.bind(this));
    }
    ...
    async handleSessionPage(req, res) {
        ...
        if (exists) {
            ...
            // 세션 폴더가 없으면 404 응답
            res.status(404).send('세션을 찾을 수 없습니다. 유효한 세션 ID인지 확인해주세요.');
        }
    }
    ...
    async handlePPTAnalysis(req, res) {
        ...
        console.log('작업 시작, 세션 ID:', sessionId);
